...
public class Task {
  ...
  static PCollection<KV<String, Iterable<String>>> applyTransform(PCollection<String> input) {
    ...
    PCollection<KV<String, String>> mapOfFirstChar = input.apply(ParDo.of(new DoFn<String, KV<String, String>>() {
            @ProcessElement
            public void processElement(@Element String word, OutputReceiver<KV<String, String>> outputReceiver) {
              outputReceiver.output(KV.of(String.valueOf(word.charAt(0)), word));
            }

    }));
    return mapOfFirstChar.apply(GroupByKey.create());
  }

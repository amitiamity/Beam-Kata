...
public class Task {
  ...
  static PCollection<Person> applyTransform(
    ...
    return persons.apply(ParDo.of(new DoFn<Person, Person>() {
      ...
      @ProcessElement
      public void processElement(@Element Person person,
                                 OutputReceiver<Person> outputReceiver, ProcessContext processContext) {
        Map<String, String> cityCountryView = processContext.sideInput(citiesToCountriesView);
        Person newPerson = new Person(person.getName(), person.getCity(), cityCountryView.get(person.getCity()));
        outputReceiver.output(newPerson);
      }

...
public class Task {
  ...
  static PCollectionTuple applyTransform(
    ...
    return numbers.apply(ParDo.of(new DoFn<Integer, Integer>() {
      @ProcessElement
      public void processElement(@Element Integer num, MultiOutputReceiver outputReceiver) {

        if ( num <= 100 ) {
          outputReceiver.get(numBelow100Tag).output(num);
        }else {
          outputReceiver.get(numAbove100Tag).output(num);
        }
      }
    }).withOutputTags(numAbove100Tag, TupleTagList.of(numBelow100Tag))) ;
  }
